#*******************************************
#
# Flexible automatic transitions for chiliproject
#
# (C) Thomas Bruderer
#
...
#***** Rule: Due day reached *****
   rule = Rules.new("Due date reached, new due date set to 2 days in the future. Please handle this ticket immediatly", "*", ["New", "In Progress", "Feedback"], 0)
        ...
        Issue.find(:all, :conditions => ["status_id IN (?) and tracker_id IN (?) and updated_on < ?", rule.status, rule.tracker, (DateTime.now - rule.age)]).each do |issue|
          ...
          if fulfilled
            ...
            if rule.changes.count > 0
              ...
              issue.journal_user = admin
              issue.journal_notes = rule.name
